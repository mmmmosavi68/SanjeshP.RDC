...
(function ($) {
    $.fn.bstreetable = function (options) {
        ...
        var settings = {
            ...
            text: {
                NodeDeleteText: "آیا از حذف مطمئن هستید؟"
            }
        };
        var TREENODECACHE = "treenode";
        var language = {};
        language.addchild = "افزودن زیر منو جدید";
        if (options) {
        ...
        /*render data*/
        var dom_addFirstLevel = $("<div class='tt-operation m-b-sm'></div>").append($("<button class='btn btn-primary btn-sm j-addClass'><i class='fa fa-level-down'></i>افزودن</button>"));
        ...
        element.delegate(".j-remove", "click", function (event) {
            ...
            if (parentDom.attr("data-loaded") == "true") {
                if (parentDom.parent().find(".class-level").length > 0) {
                    settings.customalert("نمیتوان حذف کرد!");
                    return;
                }
                else {
                    isRemoveAble = true;
        ...
        function renderHeader(_dom_header) {
            ...
            dom_row.append($("<span class='textalign-center'>عملیات</span>"));
