...
Begin
	...
	LEFT OUTER JOIN dbo.AccessMenusGroups AS AMG_User ON AMG_User.ListMenuId=M.Id and AMG_User.GroupId in (select UG.GroupId 
																							from UserGroups as UG
																							where UG.UserId=@CurrentUserId 
																							and UG.GroupId=@GroupSelected_GroupId
																							and UG.IsActive=1 
																							and UG.IsDelete=0)
	LEFT OUTER JOIN dbo.AccessMenusGroups AS AMG_Group ON AMG_Group.ListMenuId=M.Id and AMG_Group.GroupId=@GroupSelected_GroupId
...
Begin
	...
	FROM dbo.Menus AS M 
	LEFT OUTER JOIN dbo.AccessMenus AS AM ON AM.ListMenuId=M.Id and AM.UserId=@CurrentUserId
	LEFT OUTER JOIN dbo.AccessMenusGroups AS AMG ON AMG.ListMenuId=M.Id and AMG.GroupId in (select UG.GroupId 
																							from UserGroups as UG
...
Begin
	...
	LEFT OUTER JOIN dbo.AccessMenusGroups AS AMG_UserSelected_Group ON AMG_UserSelected_Group.ListMenuId=M.Id and AMG_UserSelected_Group.GroupId in (select UG.GroupId 
																							from UserGroups as UG
